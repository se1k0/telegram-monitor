...
# 获取数据库文件路径
DB_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "data", "telegram_data.db")
...
async def create_tokens_mark_table_async() -> bool:
    """
    异步创建tokens_mark表
    ...
    Returns:
        bool: 表是否创建成功
    """
    return create_tokens_mark_table()
...
async def main_async():
    ...
    if check_tokens_mark_table():
        ...
        result = await create_tokens_mark_table_async()
        if result:
            logger.info("tokens_mark表创建成功")
            return True
        else:
