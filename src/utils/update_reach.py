...
from src.database.supabase_adapter import SupabaseAdapter, get_adapter
...
# 状态文件路径，用于断点续传
STATUS_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 
                          "data", "community_reach_status.json")
...
async def get_token_community_data(token_symbol: str) -> Dict[str, Any]:
    """
    获取单个代币的社区数据
    
    Args:
        token_symbol: 代币Symbol
    ...
    try:
        # 使用Supabase获取所有相关的token_marks记录
        ...
        if not token_marks or isinstance(token_marks, dict) and token_marks.get('error'):
            logger.warning(f"获取代币(Symbol: {token_symbol})的标记记录失败")
            result["error"] = "未找到代币标记记录"
            return result
            
        # 获取唯一的channel_id
        unique_channel_ids = set()
        for mark in token_marks:
            if mark and 'channel_id' in mark and mark['channel_id']:
                unique_channel_ids.add(mark['channel_id'])
        ...
        for channel_id in unique_channel_ids:
            ...
            # 只考虑活跃频道
            if not channel_info.get('is_active', False):
                continue
                
            active_channels.add(channel_id)
            
            member_count = channel_info.get('member_count', 0) or 0
            if member_count > 0:
            ...
            channel_details.append({
                ...
                "username": channel_info.get('channel_username', ''),
        ...
        logger.error(f"获取代币(Symbol: {token_symbol})社区数据时出错: {str(e)}")
        import traceback
        logger.debug(traceback.format_exc())
        result["error"] = str(e)
        return result

async def update_token_community_reach(token_symbol: str, community_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    更新代币的社区覆盖数据
    ...
    try:
        ...
        if not community_data:
            community_data = await get_token_community_data(token_symbol)
        ...
        if not tokens or not isinstance(tokens, list) or len(tokens) == 0:
            result["error"] = f"找不到symbol为{token_symbol}的代币"
            return result
            
        token = tokens[0]
        previous_reach = token.get('community_reach', 0) or 0
        result["previous_reach"] = previous_reach
        
        # 准备更新数据
        update_data = {
            'community_reach': community_data["total_members"],
            'spread_count': community_data["spread_count"]
        }
        ...
        if isinstance(update_result, dict) and update_result.get('error'):
            logger.error(f"更新代币社区覆盖数据失败: {update_result.get('error')}")
        ...
        logger.info(f"已更新代币(symbol: {token_symbol})的社区覆盖数据: "
        ...
        logger.error(f"更新代币(symbol: {token_symbol})社区覆盖数据时出错: {str(e)}")
        import traceback
        logger.debug(traceback.format_exc())
        result["error"] = str(e)
        return result

async def update_token_community_reach_async(token_symbol: str) -> Dict[str, Any]:
    """
    异步更新代币的社区覆盖数据
    ...
    try:
        ...
        # 更新社区覆盖数据
        update_result = await update_token_community_reach(token_symbol, community_data)
        
        result.update(update_result)
        return result
        
    except Exception as e:
        logger.error(f"异步更新代币(符号: {token_symbol})社区覆盖数据时出错: {str(e)}")
        import traceback
        logger.debug(traceback.format_exc())
        result["error"] = str(e)
        return result

async def aggregate_member_counts(token_symbol: str) -> Dict[str, Any]:
    """
    聚合特定代币相关频道的成员数
    ...
    try:
        logger.info(f"开始聚合代币(symbol: {token_symbol})相关频道的成员数...")
        
        # 获取所有提及该代币的tokens_mark记录
        token_marks = await db_adapter.execute_query(
            'tokens_mark',
            'select',
            filters={'token_symbol': token_symbol}
        ...
        # 更新结果
        result["total_members"] = total_members
        result["channel_count"] = len(channel_details)
        result["channels"] = channel_details
        result["success"] = True
        
        logger.info(f"代币 {token_symbol} 聚合结果:")
        ...
        logger.error(f"聚合代币(Symbol: {token_symbol})频道成员数时出错: {str(e)}")
        import traceback
        logger.debug(traceback.format_exc())
        result["error"] = str(e)
        return result

async def update_community_reach_and_spread(token_symbol: str, aggregated_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    更新代币的社区覆盖和传播计数
    ...
    try:
        ...
        if not aggregated_data:
            aggregated_data = await aggregate_member_counts(token_symbol)
        ...
        # 准备更新数据
        update_data = {
            'community_reach': aggregated_data["total_members"],
            'spread_count': aggregated_data["channel_count"]
        }
        ...
        # 更新结果
        result["new_reach"] = aggregated_data["total_members"]
        result["change"] = aggregated_data["total_members"] - previous_reach
        result["spread_count"] = aggregated_data["channel_count"]
        result["success"] = True
        
        logger.info(f"已更新代币 {token_symbol} 的社区覆盖数据:")
        logger.info(f"社区覆盖: {result['new_reach']} (变化: {result['change']:+})")
        logger.info(f"传播计数: {result['spread_count']}")
        
        return result
        
    except Exception as e:
        logger.error(f"更新代币(Symbol: {token_symbol})社区覆盖和传播计数时出错: {str(e)}")
        import traceback
        logger.debug(traceback.format_exc())
        result["error"] = str(e)
        return result

async def update_all_tokens_community_reach(limit: Optional[int] = None, 
    ...
    try:
        # 获取所有需要更新的代币
        tokens_query = await db_adapter.execute_query(
            'tokens',
            'select',
            fields=['token_symbol']
        )
        
        if not tokens_query or isinstance(tokens_query, dict) and tokens_query.get('error'):
            logger.error(f"获取代币列表失败: {tokens_query.get('error') if isinstance(tokens_query, dict) else '未知错误'}")
            result["error"] = "获取代币列表失败"
            return result
            
        # 提取代币符号
        tokens = []
        for token in tokens_query:
            if token and 'token_symbol' in token and token['token_symbol']:
                tokens.append(token['token_symbol'])
                
        # 如果从上次中断的位置继续，排除已完成的代币
        if continue_from_last and completed_tokens:
            tokens = [t for t in tokens if t not in completed_tokens]
        ...
        for i, token_symbol in enumerate(tokens):
            try:
                # 获取社区数据
                community_data = await get_token_community_data(token_symbol)
                ...
                if update_result["success"]:
                    result["success"] += 1
                    completed_tokens.add(token_symbol)
                else:
                    result["failed"] += 1
                ...
                logger.error(f"更新代币(符号: {token_symbol})时出错: {str(e)}")
...
async def update_all_tokens_community_reach_async(limit: Optional[int] = None, 
    ...
    async def process_token(token_symbol: str) -> Dict[str, Any]:
        """处理单个代币的异步函数"""
        async with semaphore:  # 使用信号量控制并发
            return await update_token_community_reach_async(token_symbol)
    
    # 创建所有任务
    tasks = [process_token(token_symbol) for token_symbol in tokens]
    ...
    for i in range(0, len(tasks), batch_size):
        ...
        for res in batch_results:
            ...
            if res.get("success", False):
                result["success"] += 1
                completed_tokens.add(res["token_symbol"])
            else:
                result["failed"] += 1
...
async def main_async():
    ...
    if args.command == 'token':
        # 更新单个代币
        aggregated_data = await aggregate_member_counts(args.token_symbol)
        if not aggregated_data["success"]:
            logger.error(f"获取代币(Symbol: {args.token_symbol})的聚合数据失败: {aggregated_data['error']}")
            return 1
            
        result = await update_community_reach_and_spread(args.token_symbol, aggregated_data)
        ...
        if hasattr(args, 'use_async') and args.use_async:
            ...
            # 使用同步更新
            result = await update_all_tokens_community_reach(
                args.limit,
                args.continue_from_last
            )
